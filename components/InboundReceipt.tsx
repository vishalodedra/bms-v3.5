...
const InboundList: React.FC<{ onNavigate: any }> = ({ onNavigate }) => {
  ...
  const renderStatusBadge = (status: string) => {
    const styles: Record<string, string> = {
      ...
      'Completed': 'bg-slate-200 text-slate-700 border-slate-300' // V34-S3-GOV-FP-26
    };
    return (
...
const InboundDetail: React.FC = () => {
  const { id } = useParams();
  const navigate = useNavigate();
  const [flow, setFlow] = useState<AnyFlowInstance | null>(null);

  useEffect(() => {
    if (id) {
       apiFetch(`${INBOUND_FLOW_ENDPOINTS.get}?id=${id}`)
         .then(res => res.json())
         .then(json => {
            if (json.ok) setFlow(json.data);
         });
    }
  }, [id]);
  ...
  // Determine if we should allow continuing workflow
  const canContinue = ['Received', 'Serialized', 'QCPending', 'Disposition'].includes(flow.state);
  
  // Map state to next logical step for navigation
  const getNextStep = () => {
      if (flow.state === 'Received') return 'serialization';
      if (flow.state === 'Serialized') return 'qc';
      if (flow.state === 'QCPending') return 'qc'; // Logic handled in wizard
      if (flow.state === 'Disposition') return 'disposition';
      return '';
  };
          ...
          <div className="flex-1 overflow-y-auto">
            <div className="p-8 grid grid-cols-1 md:grid-cols-3 gap-8">
               ...
               <div className="space-y-4">
                  ...
                  {['Released', 'Blocked', 'Scrapped', 'Completed'].includes(flow.state) ? (
                        ...
                        <table className="w-full text-left text-xs">
                            <thead className="bg-slate-50 text-slate-500 sticky top-0">
                                <tr>
                                    ...
                                    <th className="px-4 py-2 font-medium text-right">QC Status</th>
                                    <th className="px-4 py-2 font-medium text-right">Disposition</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-100">
                                {serializedItems.map((item: any, idx: number) => (
                                    <tr key={idx}>
                                        ...
                                        <td className="px-4 py-2 text-right">
                                            {item.disposition ? (
                                                <span className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase ${
                                                    item.disposition === 'RELEASED' ? 'bg-green-50 text-green-700 border border-green-100' :
...
export const InboundReceipt: React.FC<InboundReceiptProps> = ({ onNavigate }) => {
  ...
  const handleExitWizard = () => {
    navigate('/stores/inbound');
  };

  return (
    <Routes>
       <Route index element={<InboundList onNavigate={onNavigate} />} />
       ...
       <Route path="new" element={
         <div className="h-full flex flex-col">
            ...
            <div className="flex-1 min-h-0">
               <InboundFlowWizard instanceId={null} onExit={handleExitWizard} />
            </div>
         </div>
       } />
       ...
       {/* Wizard Routes for resuming specific steps */}
       <Route path=":id/*" element={
          <div className="h-full flex flex-col">
            <div className="mb-4">
              <button onClick={() => navigate('..')} className="text-xs text-slate-500 hover:text-brand-600 flex items-center gap-1 font-medium">
                  <ArrowLeft size={12} /> Cancel Flow
              </button>
            </div>
             {/* Wrapper to extract ID and pass to wizard */}
             <WizardRouteWrapper onExit={handleExitWizard} />
          </div>
       } />
    </Routes>
  );
...
const WizardRouteWrapper: React.FC<{ onExit: () => void }> = ({ onExit }) => {
  const { id } = useParams();
  return (
    <div className="flex-1 min-h-0">
       <InboundFlowWizard instanceId={id} onExit={onExit} />
    </div>
  );
};
